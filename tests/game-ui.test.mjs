...
test("release 1.5.3 keeps product metadata while content and save formats advance", () => {
  const pkg = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8"));
  assert.equal(RELEASE_NAME, "1.5.3");
  assert.equal(RELEASE_VERSION, "1.5.3");
  assert.equal(pkg.version, RELEASE_VERSION);
  assert.equal(GAME_SYSTEM_VERSION, 2);
  assert.equal(SAVE_VERSION, 13);
  assert.ok(LATEST_UPDATE.title.startsWith("1.5.3"));
