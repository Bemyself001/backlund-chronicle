...
const CharacterPanel = memo(function CharacterPanel({ game }) {
  ...
  return <div className={styles.panelContent}>
    <div className={styles.profile}><div className={styles.avatar} aria-hidden="true"><i /><b /></div><div><p data-extraordinary={advancement.type === "extraordinary"}>{advancement.type === "extraordinary" ? `非凡者 · ${advancement.pathwayName} · ${advancement.sequenceLabel}` : "普通人"}</p><h2>{character.name}</h2><span>{character.occupation} · {character.age}岁</span></div></div>
...
const InventoryPanel = memo(function InventoryPanel({ game, onLocalTool, onAction, disabled }) {
  ...
  const advancementEligibility = selected ? getPotionAdvancementEligibility(game, selected.instanceId) : null;
  const canPromoteWithSelected = Boolean(advancementEligibility);
  ...
  return <div className={styles.rightPanel}>
    ...
    {tab === "inventory" && <div className={styles.panelContent}>
      ...
      <div className={styles.itemList}>{items.length === 0 ? <div className={styles.empty}>没有符合条件的物品。</div> : items.map((item) => <button key={item.instanceId} className={`${styles.item} ${item.isNew ? styles.newItem : ""}`} type="button" onClick={() => setSelectedId(item.instanceId)}><span className={styles.itemGlyph}>{item.name.slice(0, 1)}</span><span><strong>{item.name}{item.equipped && <small>已装备</small>}{item.potion && <small>{item.potion.identified ? `序列${item.potion.sequence}` : "未鉴定"}</small>}</strong><em>{item.category} · {item.condition}</em></span><b>×{item.quantity}</b></button>)}</div>
      {selected && <div className={styles.itemDetail}><button type="button" aria-label="关闭物品详情" onClick={() => setSelectedId(null)}>×</button><p>{selected.rarity} · {selected.category}</p><h3>{selected.name}</h3><span>{selected.description}</span>{selected.potion && <div className={`${styles.potionRecord} ${selected.potion.identified ? styles.potionIdentified : ""}`}><strong>{selected.potion.identified ? `${selected.potion.pathwayName}途径 · 序列${selected.potion.sequence}魔药` : "性质未明的魔药"}</strong><small>{selected.potion.identified ? canPromoteWithSelected ? "配方与当前序列已经匹配，可以申请本地晋升确认。" : "已完成鉴定，但仍需匹配配方、途径与目标序列。" : "需要对应配方或同途径经验才能鉴定；不能直接服用。"}</small></div>}<dl><div><dt>重量</dt><dd>{selected.weight} kg</dd></div><div><dt>状态</dt><dd>{selected.condition}</dd></div><div><dt>来源</dt><dd>{selected.source}</dd></div></dl><div className={styles.itemActions}>{canPromoteWithSelected && <button className={styles.promote} type="button" disabled={disabled} onClick={() => { setSelectedId(null); onAction(`服用${selected.name}并正式晋升至${selected.potion.pathwayName}序列${selected.potion.sequence}`, { advancementRequest: { potionInstanceId: selected.instanceId } }); }}>服用并晋升</button>}<button type="button" disabled={disabled} onClick={() => onLocalTool("item.inspect", { instanceId: selected.instanceId }, `检查${selected.name}`)}>检查</button>{selected.tags.includes("消耗品") && !selected.potion && <button type="button" disabled={disabled} onClick={() => onLocalTool("item.use", { instanceId: selected.instanceId }, `主动使用${selected.name}`)}>使用</button>}{selected.tags.includes("装备") && <button type="button" disabled={disabled} onClick={() => onLocalTool(selected.equipped ? "item.unequip" : "item.equip", { instanceId: selected.instanceId }, `玩家${selected.equipped ? "卸下" : "装备"}${selected.name}`)}>{selected.equipped ? "卸下" : "装备"}</button>}<button className={styles.discard} type="button" disabled={disabled} onClick={() => { if (window.confirm(`丢弃一件“${selected.name}”？`)) { onLocalTool("inventory.remove", { instanceId: selected.instanceId, quantity: 1 }, `玩家主动丢弃${selected.name}`); setSelectedId(null); } }}>丢弃</button></div></div>}
    </div>}
...
function AuditPanel({ game }) {
  ...
  const changeRows = [
    ...
    ...(inventory?.updated || []).map((item) => ({ tone: "update", text: `更新「${item.name}」：${item.fields.map((field) => field === "potion" ? "魔药鉴定" : field).join("、")}` })),
  ];
  return <div className={`${styles.panelContent} ${styles.auditPanel}`}>
    ...
    {audit && <>
      ...
      {importantConfirmation?.required && <div className={styles.auditNote}>{importantConfirmation.advancement ? importantConfirmation.advancement.status === "confirmed" ? `晋升确认：已成为${importantConfirmation.advancement.target.pathwayName}${importantConfirmation.advancement.target.sequenceLabel}` : "晋升确认：玩家选择暂不服用，魔药已保留" : `重要物品确认：已确认 ${importantConfirmation.confirmed} 项${importantConfirmation.rejected ? `，拒绝 ${importantConfirmation.rejected} 项` : ""}`}</div>}
...
export default function GameScreen({ game, loading, turnPhase, streamText, error, onAction, onAbort, onRetry, onRegenerateChoices, onLocalTool, onOpenMap, onOpenApi, onOpenPrompt, onOpenSaves, onHome }) {
  ...
  return <main className={styles.shell} id="main">
    <header className={styles.topbar}>
      <button className={styles.wordmark} type="button" onClick={onHome} aria-label="返回欢迎页"><i>BC</i><span>诡秘之主：贝克兰德纪事<small>BACKLUND CHRONICLE</small></span></button>
    ...
    <div className={styles.workspace}>
      ...
      <aside className={`${styles.right} ${mobilePanel === "inventory" ? styles.drawerOpen : ""}`} aria-label="物品与档案"><div className={styles.drawerHeader}><span>物品与档案</span><button type="button" onClick={() => setMobilePanel(null)}>关闭</button></div><InventoryPanel game={game} onLocalTool={onLocalTool} onAction={onAction} disabled={loading} /></aside>
