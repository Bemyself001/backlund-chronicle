    ...
    <main className={styles.page} id="main">
      ...
      <section className={styles.layout}>
        ...
        <form className={styles.form} onSubmit={submit}>
          ...
          {character.extraordinary === "low" && <label className={`${styles.field} ${styles.pathwayField}`}><span>序列9途径</span><select value={character.pathway} onChange={(e) => update("pathway", e.target.value)} required aria-describedby="pathway-help">{LOW_SEQUENCE_PATHWAYS.map((pathway) => <option key={pathway} value={pathway}>{pathway}</option>)}</select><small id="pathway-help">已开放全部22条神之途径；非凡能力同时伴随失控与暴露风险。</small></label>}
          </fieldset>
