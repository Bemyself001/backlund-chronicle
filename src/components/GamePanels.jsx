...
export const CharacterPanel = memo(function CharacterPanel({ game }) {
  ...
  return <div className={styles.content}>
    ...
    <section><h3>角色状态</h3><dl className={styles.dataList}>{Object.entries(STAT_LABELS).map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{character.stats[key]} / {character.stats[`max${key[0].toUpperCase()}${key.slice(1)}`]}</dd></div>)}</dl></section>
