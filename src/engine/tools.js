...
function executeOne(game, call, options = {}) {
  ...
  switch (call.name) {
    ...
    case "trigger.engage": {
      if (options.playerAction !== undefined && !/(追查|继续|深入|验证|调查|接受|着手|查明|拆查|检查|回应|接取|救助|救治|解读|辨认|查阅|请教|打听)/.test(String(options.playerAction))) return fail(call.name, "玩家本轮没有明确表示追查该事件");
    ...
    case "status.add": {
      ...
      const { tick: _rawTick, ...statusFields } = args.status;
      game.statusEffects.push({ kind: "neutral", description: "", ...statusFields, ...(Object.keys(tick).length ? { tick } : {}) });
